...
// apps/obs-views/src/scoreboard-animations.ts
var ScoreboardAnimator = class {
  elements;
  lowerThirdTimeline = null;
  ...
  animateScoreChange(scoreElement, nextScoreText) {
    const timeline2 = gsapWithCSS.timeline();
    timeline2.to(scoreElement, {
      scale: 1.22,
      duration: 0.11,
      ease: "power2.out"
    });
  ...
  playEvent(event) {
    ...
    if (event.kind === "featured_player" || event.kind === "player_spotlight" || event.kind === "player_focus" || event.kind === "player_info") {
      this.playFeaturedPlayerEvent(event);
      return;
    }
    if (event.kind === "info" || event.kind === "announcement" || event.kind === "generic_info" || event.kind === "generic") {
  ...
  playInfoEvent(event) {
    ...
    const timeline2 = gsapWithCSS.timeline({
      onComplete: () => {
        gsapWithCSS.set(infoRail, { clearProps: "all" });
  ...
  eventLooksLikePlayerSpotlight(event) {
    const meta = this.getMetaObject(event.meta);
    return Boolean(
      meta.player || meta.player_name || meta.first_name || meta.last_name || meta.points || meta.goals || meta.assists
    );
  }
  isPossessionOnlyEvent(event) {
  ...
  resolveTimeoutPrimaryText(event) {
  ...
  extractPlayerDetails(event) {
    ...
    const name = this.cleanText(
      event.primary_text ?? metaPlayer.name ?? meta.player_name ?? meta.name ?? (fullNameCandidate || void 0),
      "Unknown Player"
    );
    ...
    const statsLine = stats.length > 0 ? stats.join("  \xB7  ") : "LIVE PLAYER SNAPSHOT";
    const note = this.cleanText(
      event.secondary_text ?? metaPlayer.note ?? meta.note ?? metaPlayer.extra_info ?? meta.extra_info ?? "",
      ""
    );
...
// apps/obs-views/src/main.ts
var viewType = document.body.dataset.viewType ?? "scoreboard";
var scoreboardAnimator = new ScoreboardAnimator();
async function applyObsBgColor() {
  try {
...
var lastPossessionTeam = null;
var seenOverlayEventIds = /* @__PURE__ */ new Set();
var seenOverlayEventOrder = [];
...
function markOverlayEventSeen(id) {
...
function applyPossessionIndicator(possessionTeamRaw) {
  ...
  const possessionTeam = isTeamKey(possessionTeamRaw) ? possessionTeamRaw : null;
  if (!possessionTeam) {
...
function renderTimeoutDots(elementId, value) {
...
function animateScoreIfChanged(elementId, score, animate) {
...
function ensureStatRow(target, key) {
  let row = target.querySelector(`.stat-row[data-key="${key}"]`);
  if (row) {
    return row;
  }
...
function renderStatBarGrid(targetId, rows) {
  const target = document.getElementById(targetId);
  if (!target) return;
  const keys = new Set(rows.map((row) => row.key));
  target.querySelectorAll(".stat-row").forEach((row) => {
  ...
  for (const rowData of rows) {
    ...
    const homeValue = row.querySelector(".home-value");
    const awayValue = row.querySelector(".away-value");
    const homeFill = row.querySelector(".home-fill");
    const awayFill = row.querySelector(".away-fill");
...
function ensureAdvancedComparisonRow(target, key) {
  let row = target.querySelector(`.advanced-row[data-key="${key}"]`);
  if (row) {
    return row;
  }
  row = document.createElement("article");
...
function renderAdvancedComparisonGrid(targetId, rows) {
  const target = document.getElementById(targetId);
  if (!target) return;
  const keys = new Set(rows.map((row) => row.key));
  target.querySelectorAll(".advanced-row").forEach((row) => {
    if (!keys.has(row.dataset.key || "")) {
      row.remove();
    }
  });
  for (const rowData of rows) {
    ...
    const labelEl = row.querySelector(".advanced-label");
    const homeEl = row.querySelector(".advanced-home");
    const awayEl = row.querySelector(".advanced-away");
...
function setStatsTeamLabels(homeName, awayName) {
...
function renderRoster(tableId, roster) {
...
function renderPlayerStats(tableId, stats) {
  const table = document.getElementById(tableId);
  if (!table) return;
  ...
  table.innerHTML = `
    ...
    <tbody>
      ${rows.map(
    ([number, line]) => `<tr><td>#${number} ${line.name}</td><td>${line.goals}</td><td>${line.assists}</td><td>${line.total}</td></tr>`
  ).join("")}
    </tbody>
  `;
  flashUpdate(table);
...
function processOverlayEvents(previousSnapshot, nextSnapshot, seedOnly) {
...
function render3(snapshot, animate = true) {
